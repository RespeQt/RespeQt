...
bool SimpleDiskImage::writeHappyProTrack(int trackNumber, bool happy1050)
{
    ...
    while (offset < (startOffset + 0x100)) {
        ...
        if (code < 128) {
            if (((quint8)m_happyRam[offset] == invertedTrack) && ((quint8)m_happyRam[offset + 1] == (quint8)0xFF) && ((quint8)m_happyRam[offset + 2] >= 0xED)
                    && ((quint8)m_happyRam[offset + 3] == (quint8)0xFF) && ((quint8)m_happyRam[offset + 4] == (quint8)0x08)) {
...
bool SimpleDiskImage::writeHappyProSectors(int trackNumber, int afterSectorNumber, bool happy1050)
{
    ...
    bool sync = (quint8) m_happyRam[0x280] == 0;
            ...
            if ((sectorNumber > 0) && (sectorNumber <= m_geometry.sectorsPerTrack())) {
                ...
                for (int i = 0; i < m_sectorsInTrack; i++) {
                    ...
                    if (sector == sectorNumber) {
                        ...
                        QByteArray data = m_happyRam.mid(dataOffset,128);
                        for (int j = 0; j < 128; j++) {
...
bool SimpleDiskImage::readProSkewAlignment(quint16 aux, QByteArray &data, bool timingOnly)
{
    ...
    if (!timingOnly) {
        ...
        m_trackData[0xF5] = 0xFF - (nbMicroSeconds / 4352) + 1;
        m_trackData[0] = m_deviceNo;
        m_trackData[1] = 0x74;
        m_trackData[2] = (quint8)firstTrack;
        m_trackData[3] = (quint8)secondTrack;
...
bool SimpleDiskImage::writeProTrackWithSkew(quint16 aux, const QByteArray &) {
    ...
    return writeProTrack((aux & 0x3F) | 0xF000, m_trackData);
}
